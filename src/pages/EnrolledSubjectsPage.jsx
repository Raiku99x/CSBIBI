...
const SUBJECT_COLORS = [
  { bg: '#E0F0F0', icon: '#0D7377', bar: 'linear-gradient(135deg,#0D7377,#0A5C60)' },
  { bg: '#DDE4EA', icon: '#3D5166', bar: 'linear-gradient(135deg,#3D5166,#2C3E50)' },
  { bg: '#D8EDE6', icon: '#2D6A4F', bar: 'linear-gradient(135deg,#2D6A4F,#1B4332)' },
  { bg: '#DCEAF5', icon: '#2E5F8A', bar: 'linear-gradient(135deg,#2E5F8A,#1A3A5C)' },
  { bg: '#EDE4D8', icon: '#7A5C42', bar: 'linear-gradient(135deg,#7A5C42,#4A3728)' },
  { bg: '#E4E0EB', icon: '#5C4A7A', bar: 'linear-gradient(135deg,#5C4A7A,#3D2B5E)' },
]
...
export default function EnrolledSubjectsPage() {
  ...
  return (
    <div style={{ paddingTop: 12 }}>
      <div style={{ background:'white', borderRadius:12, border:'1px solid #DADDE1', padding:'16px 20px', marginBottom:8, boxShadow:'0 1px 2px rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10, background:'#F0F2F5', borderRadius:22, padding:'0 14px', height:40 }}>
          ...
          <input type="text" placeholder="Search subjects…" value={search} onChange={e => setSearch(e.target.value)}
            style={{ flex:1, border:'none', background:'transparent', outline:'none', fontFamily:'"Instrument Sans",system-ui', fontSize:14, color:'#050505' }}/>
          ...
          {enrolledSubjects.length > 0 && (
            <Section title="Enrolled" count={enrolledSubjects.length}>
              {enrolledSubjects.map(s => (
                <SubjectRow key={s.id} subject={s} enrolled toggling={toggling===s.id} onToggle={() => toggle(s.id, true)} onClick={() => setSelected(s)}/>
              ))}
            </Section>
          )}
          {availableSubjects.length > 0 && (
            <Section title={enrolledIds.size > 0 ? 'Available to Join' : 'All Subjects'} count={availableSubjects.length}>
              {availableSubjects.map(s => (
                <SubjectRow key={s.id} subject={s} enrolled={false} toggling={toggling===s.id} onToggle={() => toggle(s.id, false)} onClick={() => setSelected(s)}/>
              ))}
            </Section>
          )}
  ...
  return (
    <div style={{ marginBottom:8 }}>
      <div style={{ display:'flex', alignItems:'center', gap:8, padding:'0 4px', marginBottom:6 }}>
        <span style={{ fontFamily:'"Instrument Sans",system-ui', fontWeight:700, fontSize:12, color:'#65676B', textTransform:'uppercase', letterSpacing:0.6 }}>{title}</span>
        <span style={{ background:'#E4E6EB', color:'#65676B', fontFamily:'"Instrument Sans",system-ui', fontWeight:700, fontSize:11, padding:'1px 7px', borderRadius:10 }}>{count}</span>
      </div>
...
function SubjectRow({ subject, enrolled, toggling, onToggle, onClick }) {
  ...
  return (
    <div style={{ background:'white', borderRadius:12, border:`1.5px solid ${enrolled?'#7EC8C8':'#DADDE1'}`, padding:'12px 14px', display:'flex', alignItems:'center', gap:12, boxShadow:'0 1px 2px rgba(0,0,0,0.05)' }}>
      <div onClick={onClick} style={{ width:44, height:44, borderRadius:12, flexShrink:0, background:color.bg, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}>
        <BookOpen size={20} color={color.icon}/>
      </div>
      <div style={{ flex:1, minWidth:0, cursor:'pointer' }} onClick={onClick}>
        <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
          <span style={{ fontFamily:'"Instrument Sans",system-ui', fontWeight:700, fontSize:14, color:'#050505' }}>{subject.name}</span>
          {enrolled && <span style={{ background:'#F0FDF4', color:'#16a34a', fontFamily:'"Instrument Sans",system-ui', fontWeight:700, fontSize:10, padding:'2px 7px', borderRadius:10 }}>✓ Enrolled</span>}
        </div>
        {subject.description && <p style={{ margin:'2px 0 0', fontFamily:'"Instrument Sans",system-ui', fontSize:12, color:'#65676B', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{subject.description}</p>}
      </div>
      <button onClick={onToggle} disabled={!!toggling}
        onMouseEnter={() => setBtnHovered(true)} onMouseLeave={() => setBtnHovered(false)}
        style={{ display:'flex', alignItems:'center', gap:5, padding:'7px 14px', borderRadius:8, border:'none', cursor:toggling?'not-allowed':'pointer', fontFamily:'"Instrument Sans",system-ui', fontWeight:700, fontSize:13, background:enrolled?(btnHovered?'#FEE2E2':'#FFF1F2'):(btnHovered?'#CCE9E9':'#E6F4F4'), color:enrolled?'#e11d48':'#0D7377', flexShrink:0, transition:'background 0.12s' }}>
        {toggling ? <Loader2 size={13} style={{ animation:'spin 0.8s linear infinite' }}/> : enrolled ? <Minus size={13}/> : <Plus size={13}/>}
        {enrolled ? 'Leave' : 'Join'}
      </button>
    </div>
  )
...
function SubjectDetail({ subject, isEnrolled, userId, onBack, onToggle }) {
  ...
  return (
    <div style={{ paddingTop:12 }}>
      <button onClick={onBack} style={{ display:'flex', alignItems:'center', gap:6, marginBottom:10, background:'none', border:'none', cursor:'pointer', padding:'4px 4px', fontFamily:'"Instrument Sans",system-ui', fontWeight:600, fontSize:14, color:'#0D7377' }}>
        <ChevronLeft size={17}/> All Subjects
      </button>

      <div style={{ background:'white', borderRadius:12, border:'1px solid #DADDE1', overflow:'hidden', marginBottom:8, boxShadow:'0 1px 2px rgba(0,0,0,0.06)' }}>
        <div style={{ height:6, background:color.bar }}/>
        <div style={{ padding:'16px 16px 0' }}>
          <div style={{ display:'flex', alignItems:'flex-start', gap:12, marginBottom:14 }}>
            <div style={{ width:52, height:52, borderRadius:14, flexShrink:0, background:color.bg, display:'flex', alignItems:'center', justifyContent:'center' }}>
        ...
        <div style={{ display:'flex', borderTop:'1px solid #F0F2F5', overflowX:'auto' }}>
          {TABS.map(({ key, label, icon: Icon }) => (
            <button key={key} onClick={() => setActiveTab(key)}
              style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', gap:5, padding:'12px 8px', border:'none', cursor:'pointer', background:'transparent', fontFamily:'"Instrument Sans",system-ui', fontWeight:600, fontSize:13, color:activeTab===key?'#0D7377':'#65676B', borderBottom:`2px solid ${activeTab===key?'#0D7377':'transparent'}`, whiteSpace:'nowrap', transition:'color 0.15s,border-color 0.15s' }}>
              <Icon size={14}/> {label}
            </button>
          ))}
        </div>
      ...
      {!isEnrolled ? (
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>{[0,1].map(i => <PostSkeleton key={i}/>)}</div>
      ) : (
        <TabContent
          activeTab={activeTab}
          posts={posts}
...
function ToggleBtn({ enrolled, onToggle }) {
  ...
  return (
    <button onClick={onToggle} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ display:'flex', alignItems:'center', gap:5, padding:'8px 16px', borderRadius:8, border:'none', cursor:'pointer', fontFamily:'"Instrument Sans",system-ui', fontWeight:700, fontSize:13, background:enrolled?(hovered?'#FEE2E2':'#FFF1F2'):(hovered?'#CCE9E9':'#E6F4F4'), color:enrolled?'#e11d48':'#0D7377', flexShrink:0, transition:'background 0.12s' }}>
...
function TabContent({ activeTab, posts, filePosts, mediaPosts, apps, userId, color, subject, onUserClick }) {
  ...
  if (activeTab === 'media') {
    ...
    return (
      <div style={{ background:'white', borderRadius:12, border:'1px solid #DADDE1', padding:8, display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:4 }}>
        {allPhotos.map((url, i) => (
  ...
  if (activeTab === 'files') {
    return filePosts.length === 0
      ...
      : <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{filePosts.map(p => <FileCard key={p.id} post={p}/>)}</div>
  }
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
          {apps.map(app => (
            <a key={app.id} href={app.url} target="_blank" rel="noopener noreferrer"
              style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 14px', borderRadius:12, textDecoration:'none', background:'white', border:'1px solid #DADDE1' }}>
              {app.icon_url
                ? <img src={app.icon_url} style={{ width:36, height:36, borderRadius:10 }} alt={app.name}/>
                : <div style={{ width:36, height:36, borderRadius:10, background:color.bg, display:'flex', alignItems:'center', justifyContent:'center' }}><AppWindow size={18} color={color.icon}/></div>
              }
              <div style={{ minWidth:0, flex:1 }}>
                <p style={{ margin:0, fontFamily:'"Instrument Sans",system-ui', fontWeight:700, fontSize:13, color:'#050505', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{app.name}</p>
...
function FileCard({ post }) {
  ...
  return (
    <a href={post.file_url} target="_blank" rel="noopener noreferrer"
      ...
      style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 14px', borderRadius:12, textDecoration:'none', background:hovered?'#F7F8FA':'white', border:`1px solid ${hovered?'#7EC8C8':'#DADDE1'}`, transition:'all 0.15s' }}>
      <div style={{ width:40, height:40, borderRadius:10, background:'#E6F4F4', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
        <FileText size={18} color="#0D7377"/>
      </div>
      <div style={{ flex:1, minWidth:0 }}>
        <p style={{ margin:0, fontFamily:'"Instrument Sans",system-ui', fontWeight:600, fontSize:14, color:'#050505', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{post.file_name || 'Attachment'}</p>
        {post.caption && <p style={{ margin:'2px 0 0', fontFamily:'"Instrument Sans",system-ui', fontSize:12, color:'#65676B', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{post.caption}</p>}
      </div>
...
function LockedState({ onJoin }) {
  return (
    <div style={{ background:'white', borderRadius:12, border:'1px solid #DADDE1', padding:'48px 24px', textAlign:'center' }}>
      ...
      <p style={{ margin:'0 0 6px', fontFamily:'"Bricolage Grotesque",system-ui', fontWeight:700, fontSize:17, color:'#050505' }}>Enroll to see content</p>
      <p style={{ margin:'0 0 20px', fontFamily:'"Instrument Sans",system-ui', fontSize:14, color:'#65676B' }}>Join this subject to access posts, files, and apps.</p>
      <button onClick={onJoin} style={{ padding:'11px 28px', borderRadius:10, border:'none', background:'#0D7377', color:'white', cursor:'pointer', fontFamily:'"Instrument Sans",system-ui', fontWeight:700, fontSize:15, display:'inline-flex', alignItems:'center', gap:6 }}>
...
function SubjectSkeleton() {
  ...
  return (
    <div style={{ background:'white', borderRadius:12, border:'1px solid #DADDE1', padding:'12px 14px', display:'flex', alignItems:'center', gap:12, marginBottom:4 }}>
      {bar(44,44,12)}
      <div style={{ flex:1, display:'flex', flexDirection:'column', gap:7 }}>{bar('50%',14)}{bar('70%',11)}</div>
      {bar(60,34,8)}
