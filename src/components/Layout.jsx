  ...
  return (
    <div style={{
      ...
      background: isSuperadmin
        ? 'linear-gradient(135deg,#F4C430,#E6A817)'
        : 'linear-gradient(135deg,#1A5276,#0D7377)',
      boxShadow: isSuperadmin
        ? '0 2px 8px rgba(244,196,48,0.4)'
        : '0 2px 8px rgba(13,115,119,0.35)',
    }}>
...
export default function Layout({ children, onOpenSearch }) {
  ...
  const textPri    = dark ? '#E4E6EB' : '#050505'
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div className="csb-sidebar-scroll" style={{ flex: 1, overflowY: 'auto', padding: isDesktop ? '12px 10px' : '8px 10px' }}>
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
                ...
                {profile?.role === 'superadmin' && (
                  <span style={{ display:'inline-flex',alignItems:'center',gap:3,background:'#FEF9C3',color:'#92400E',border:'1px solid #FDE68A',borderRadius:10,padding:'1px 6px',fontSize:10,fontWeight:700,fontFamily:'"Instrument Sans",system-ui',flexShrink:0 }}>
                    <Crown size={9}/> Admin
                  </span>
                )}
                {profile?.role === 'moderator' && (
                  <span style={{ display:'inline-flex',alignItems:'center',gap:3,background:'#EBF5FB',color:BLUE,border:`1px solid #AED6F1`,borderRadius:10,padding:'1px 6px',fontSize:10,fontWeight:700,fontFamily:'"Instrument Sans",system-ui',flexShrink:0 }}>
                    <Shield size={9}/> Mod
                  </span>
                )}
              </div>
          ...
          {/* Mod Mode section — only for mods/admins */}
              ...
              <div style={{ marginBottom: 4 }}>
                ...
                <div style={{ display:'flex',alignItems:'center',gap:11,padding:'8px 10px',borderRadius:10,marginBottom:2 }}>
                  <div style={{ width:32,height:32,borderRadius:9,flexShrink:0,background:modMode?(isSuperadmin?'#FEF9C3':'#EBF5FB'):surfaceBg,display:'flex',alignItems:'center',justifyContent:'center' }}>
                    {isSuperadmin
                      ? <Crown size={16} color={modMode?'#92400E':textSec}/>
                      : <Shield size={16} color={modMode?BLUE:textSec}/>
                    }
                  </div>
                  <span style={{ flex:1,fontFamily:'"Instrument Sans",system-ui',fontWeight:600,fontSize:14,color:textPri }}>
                    {modMode ? 'Mod Mode ON' : 'Mod Mode OFF'}
                  </span>
...
function NotifItem({ notif, onRead, onClose, navigate, cardBg, textPri, textSec, surfaceBg }) {
  ...
  return (
    <button onClick={handleClick} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ width:'100%',display:'flex',alignItems:'flex-start',gap:10,padding:'10px 14px',border:'none',cursor:'pointer',textAlign:'left',background:hovered?surfaceBg:notif.is_read?cardBg:'#FFF8F8',borderLeft:notif.is_read?'3px solid transparent':`3px solid ${RED}`,transition:'background 0.12s' }}>
      <div style={{ width:32,height:32,borderRadius:9,flexShrink:0,background:notif.is_read?surfaceBg:'#FADBD8',display:'flex',alignItems:'center',justifyContent:'center',fontSize:15 }}>{icons[notif.type]||'🔔'}</div>
      <div style={{ flex:1,minWidth:0 }}>
