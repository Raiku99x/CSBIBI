...
function PhotoGrid({ photos, onPhotoClick }) {
  ...
  const wrap = (url, i, style={}) => (
    <div key={i} onClick={() => onPhotoClick(i)} style={{ overflow:'hidden',cursor:'pointer',position:'relative',background:'#E4E6EB',...style }}
      onMouseEnter={e=>{const img=e.currentTarget.querySelector('img');if(img)img.style.transform='scale(1.03)'}}
      onMouseLeave={e=>{const img=e.currentTarget.querySelector('img');if(img)img.style.transform='scale(1)'}}>
      <img src={url} alt="" loading="lazy" style={{ width:'100%',height:'100%',objectFit:'cover',display:'block',transition:'transform 0.25s ease, opacity 0.3s',opacity:0 }} onLoad={e => e.currentTarget.style.opacity = 1}/>
  ...
  if (count===1) return (
    <div onClick={()=>onPhotoClick(0)}
      style={{cursor:'pointer',overflow:'hidden',maxHeight:380,background:'#E4E6EB',minHeight:180}}
      onMouseEnter={e=>{const img=e.currentTarget.querySelector('img');if(img)img.style.transform='scale(1.02)'}}
      onMouseLeave={e=>{const img=e.currentTarget.querySelector('img');if(img)img.style.transform='scale(1)'}}>
      <img src={display[0]}
        style={{width:'100%',maxHeight:380,objectFit:'cover',display:'block',transition:'transform 0.25s ease, opacity 0.3s',opacity:0}}
        loading="lazy" alt=""
      ...
      <div ref={modalRef} style={{ background:colors.cardBg,borderRadius:14,width:'100%',maxWidth:300,boxShadow:'0 12px 36px rgba(0,0,0,0.18)',overflow:'hidden',animation:'expandIn 0.16s ease' }}>
        ...
        <div style={{ margin: '0 10px 6px', padding: '5px 9px', background: '#EDE9FE', borderRadius: 7, display: 'flex', alignItems: 'center', gap: 5 }}>
          ...
          <p style={{ margin: 0, fontFamily: '"Instrument Sans", system-ui', fontSize: 11, color: '#5B21B6', lineHeight: 1.3 }}>
            Only the you and selected members can see this.
          </p>
        </div>
      ...
      {postData.is_pinned && (
        ...
        <div style={{ padding:'10px 12px 8px',display:'flex',alignItems:'center',gap:9 }}>
          <img
            ...
            style={{ width:40,height:40,borderRadius:10,objectFit:'cover',flexShrink:0,border:`1.5px solid ${colors.border}`,cursor:onUserClick?'pointer':'default',background:'#E4E6EB' }}
          />
          <div style={{ flex:1,minWidth:0 }}>
